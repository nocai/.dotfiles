...
function M.on_attach(_, buffer)
  ...
  vim.keymap.set(
    "n",
    ivim.lsp_keys.Diagnostic,
    vim.diagnostic.open_float,
    { buffer = buffer, desc = "lsp: diagnostic float" }
  )
  vim.keymap.set(
    "n",
    ivim.lsp_keys.DiagnosticNext,
    M._diagnostic_goto(true),
    { buffer = buffer, desc = "lsp: diagnostic next" }
  )
  vim.keymap.set(
    "n",
    ivim.lsp_keys.DiagnosticPrev,
    M._diagnostic_goto(false),
    { buffer = buffer, desc = "lsp: diagnostic prev" }
  )
  vim.keymap.set("n", "]e", M._diagnostic_goto(true, "ERROR"), { buffer = buffer, desc = "lsp: error next" })
  vim.keymap.set("n", "[e", M._diagnostic_goto(false, "ERROR"), { buffer = buffer, desc = "lsp: error prev" })
  vim.keymap.set("n", "]w", M._diagnostic_goto(true, "WARN"), { buffer = buffer, desc = "lsp: warn next" })
  vim.keymap.set("n", "[w", M._diagnostic_goto(false, "WARN"), { buffer = buffer, desc = "lsp: warn prev" })
